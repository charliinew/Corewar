    ...
    int32_t arg[3])
{
    ...
    sum += arg[1];
    copy = reconstruct_int(corewar->memory, (champion->PC + sum)
        % MEM_SIZE, 4);
...
static int exec_lldi(corewar_t *corewar, champion_t *champion,
    int32_t arg[3], char *cdb)
{
    ...
    exec_lldi_next(corewar, champion, arg);
    return 0;
}
...
void my_lldi(corewar_t *corewar, champion_t *champion)
{
    ...
    if (check_coding_byte(coding_byte, 13) == 84) {
        free(coding_byte);
        return;
    }
    pos = (pos + 1) % MEM_SIZE;
    ...
    if (exec_lldi(corewar, champion, arg, coding_byte) != 84) {
        champion->cycle_instruction = 49;
        champion->PC = pos;
    } else
