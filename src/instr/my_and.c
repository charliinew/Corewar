...
static void exec_and(champion_t *champion, int32_t arg[3], char *cdb)
{
...
void my_and(corewar_t *corewar, champion_t *champion)
{
    ...
    if (check_coding_byte(coding_byte, 5) == 84) {
        free(coding_byte);
        return;
    }
    ...
    exec_and(champion, (int32_t[3]){arg1, arg2, arg3}, coding_byte);
    free(coding_byte);
}
